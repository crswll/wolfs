...
const QuickContact = ({
  ...
  return (
    <div className={`${parentClassNames || ''} text-center text-darker text-scale-0 font-400 items-center`} {...props}>
      ...
      <div className="flex items-center justify-center mt-1 space-x-1 text-scale-1 narrow:text-scale-3">
        <a className="mr-1 border-b border-lighter hover:border-light focus:border-light" href={`tel:${phone}`}>{phone}</a> <span>|</span> <strong>We Deliver!</strong>
      </div>
    </div>
  )
}
